...
#4a : trim any leading or following spaces
update ml_stage
...
#4b: check for potential misspellings in different entries
...
-- step X: check if any of the voters voted on themselves
select *
from ml_stage
...
-- creating ID columns
# the round_names column already has a definite order, so we'll start with that first
alter table ml_stage
...
# assigning ID numbers to participants
alter table ml_stage
...
#lookup how to update with a join
...
# adding a "vote_type" column next to the "vote_value" column
alter table ml_stage
...
-- splitting the clean table into smaller specific tables to consolidate
#"round_names" table
...
create table ml_votes (
	select round_id, song_title, submitter_id, voter_id, vote_type
	from ml_stage);
